...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const storageCart = JSON.parse(localStorage.getItem('minka_cart') || 'null')
    const savedItems = reduxCartItems.length > 0
      ? reduxCartItems
      : storageCart?.items || []
  ...
  const selectedLocation = shippingLocations.find((location) => location._id === selectedLocationId)
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const result = await validateCoupon(couponCode.trim().toUpperCase(), totalBeforeCoupon)
      setCoupon(result)
      setCouponMessage(`✨ ${result.code} applied! You saved ${formatNaira(result.discount)}`)
    } catch (error) {
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      setOrderError(error?.message || error || 'Failed to place order. Please try again.')
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 pt-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-4xl md:text-5xl font-light tracking-tight text-gray-900">Checkout</h1>
          <p className="text-gray-500 mt-2 font-light">Complete your order with confidence</p>
        </div>
              ...
              <div className="p-6">
                <div className="space-y-3">
                  <label className={`flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all ${
                    formData.paymentMethod === 'card' ? 'border-amber-400 bg-amber-50' : 'border-gray-200 hover:border-gray-300'
                  }`}>
                    <input
                      type="radio"
                      name="paymentMethod"
                    ...
                    <div className="flex gap-2">
                      <img src="https://cdn-icons-png.flaticon.com/512/196/196539.png" alt="Visa" className="h-6" />
                      <img src="https://cdn-icons-png.flaticon.com/512/196/196561.png" alt="Mastercard" className="h-6" />
                    </div>
                  </label>
                  
                  <label className={`flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all ${
                    formData.paymentMethod === 'bank' ? 'border-amber-400 bg-amber-50' : 'border-gray-200 hover:border-gray-300'
                  }`}>
                    <input
                      type="radio"
                      name="paymentMethod"
                  ...
                  <label className={`flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all ${
                    formData.paymentMethod === 'cash' ? 'border-amber-400 bg-amber-50' : 'border-gray-200 hover:border-gray-300'
                  }`}>
                    <input
                      type="radio"
                      name="paymentMethod"
                ...
                <div className="max-h-80 overflow-y-auto divide-y divide-gray-100">
                  {cartItems.map((item, idx) => (
                    <div key={idx} className="p-4 flex gap-3 hover:bg-gray-50 transition-colors">
                      ...
                      <p className="font-semibold text-gray-900 text-sm">{formatNaira(item.price * item.quantity)}</p>
                    </div>
                  ))}
                </div>
