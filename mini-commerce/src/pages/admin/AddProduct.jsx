...
const AddProduct = () => {
  ...
  const handleFileSelect = async (e) => {
    ...
    if (invalidTypeFiles.length > 0) {
      setUploadError('Only JPG and PNG images are supported. Please remove the invalid file(s) and try again.')
      if (fileInputRef.current) fileInputRef.current.value = ''
      return
    }

    if (oversizedFiles.length > 0) {
      setUploadError('Each image must be smaller than 10MB. Please remove the large file(s) and try again.')
      if (fileInputRef.current) fileInputRef.current.value = ''
      return
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              {uploadingImages && (
                <p className="mt-2 text-sm text-gray-500">Do not save the product while images are still uploading.</p>
              )}
            </div>
