...
import { FaArrowLeft, FaTruck, FaCheckCircle, FaClock, FaBox, FaMapMarkerAlt, FaCreditCard, FaTag, FaUser, FaEnvelope, FaPhone, FaPrint } from 'react-icons/fa'
        ...
        <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-6 text-white">
          <div className="flex flex-wrap justify-between items-start gap-4">
            <div>
              ...
              <h1 className="text-2xl font-mono font-bold">#{order._id?.slice(-8)}</h1>
              <p className="text-gray-300 text-sm mt-2">Placed on {new Date(order.createdAt).toLocaleString()}</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-gray-300">Total Amount</p>
              <p className="text-3xl font-bold text-amber-400">{formatNaira(order.totalPrice)}</p>
            </div>
          </div>
        </div>
                  ...
                  <div key={idx} className="p-5 hover:bg-gray-50 transition-colors">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-amber-600 font-bold mt-2">{formatNaira(item.price * item.quantity)}</p>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <div className="relative flex justify-between">
                  {timelineSteps.map((step, idx) => (
                    <div key={idx} className="text-center" style={{ flex: 1 }}>
                      ...
                      <p className={`text-xs font-medium ${idx <= currentStepIndex ? 'text-gray-800' : 'text-gray-400'}`}>
                        {step.label}
                      </p>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden sticky top-24">
              ...
              <div className="p-6 space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{formatNaira(order.itemsPrice)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Shipping</span>
                  <span className="font-medium">{order.shippingPrice === 0 ? 'Free' : formatNaira(order.shippingPrice)}</span>
                </div>
                {order.coupon && (
                ...
                <div className="border-t border-gray-100 pt-3 mt-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-xl font-bold text-amber-600">{formatNaira(order.totalPrice)}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="p-6 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-800">{order.user?.email || order.customer?.email}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-800">{order.shippingAddress?.phone}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-800">{order.shippingAddress?.address}, {order.shippingAddress?.city}, {order.shippingAddress?.state}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
