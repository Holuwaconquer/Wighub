...
export const getProducts = async (filters = {}) => {
    try {
        ...
        if (filters.hairType && filters.hairType !== 'all') params.append('hairType', filters.hairType);
...
export const initializePayment = async (orderId, paymentMethod) => {
    try {
        const response = await api.post('/payment/initialize', { orderId, paymentMethod });
        return response.data;
    } catch (error) {
        throw error.response?.data || error.message;
    }
...
export const getAllOrders = async (status = 'all', page = 1) => {
    try {
        const response = await api.get(`/admin/orders?status=${status}&page=${page}`);
        return response.data;
    } catch (error) {
        throw error.response?.data || error.message;
    }
...
export const updateOrderStatus = async (orderId, status, trackingNumber = null) => {
    try {
        const response = await api.put(`/admin/orders/${orderId}/status`, { status, trackingNumber });
        return response.data;
    } catch (error) {
        throw error.response?.data || error.message;
    }
